// reactbits.dev SoftAurora (Backgrounds) — TS 이식.
// 사용 규칙(CLAUDE.md): 히어로 배경 전용. 모바일(<768px)·prefers-reduced-motion에서는
// 렌더하지 않는다 — 그 경우 Hero의 정적 CSS 메시 그라디언트(.hero-mesh)가 폴백.
...
/** WebGL 렌더 허용 조건 — 데스크톱 + 모션 허용 */
const useCanAnimate = () => {
  const [ok, setOk] = useState(false)
  useEffect(() => {
    const wide = window.matchMedia('(min-width: 768px)')
    const motion = window.matchMedia('(prefers-reduced-motion: no-preference)')
    const update = () => setOk(wide.matches && motion.matches)
    update()
...
const SoftAuroraCanvas: React.FC<Required<SoftAuroraProps>> = ({
  ...
  useEffect(() => {
    ...
    // 캔버스는 pointer-events: none (히어로 CTA 클릭 보호) — 마우스는 window에서 추적
...
const SoftAurora: React.FC<SoftAuroraProps> = ({
  ...
  scale = 1.5,
  brightness = 1.0,
  color1 = '#007cf0', // DESIGN.md develop
  color2 = '#ff0080', // DESIGN.md preview
  noiseFrequency = 2.5,
  noiseAmplitude = 1.0,
  bandHeight = 0.5,
  bandSpread = 1.0,
  ...
  if (!canAnimate) return null // 정적 CSS 메시 그라디언트가 폴백
  return (
    <SoftAuroraCanvas
      ...
      octaveDecay={octaveDecay} layerOffset={layerOffset} colorSpeed={colorSpeed}
